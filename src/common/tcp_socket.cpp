...
TcpSocket TcpSocket::listen_on(uint16_t port, int backlog) {
    ...
    if (fd < 0) fail("socket failed to create");
...
TcpSocket TcpSocket::accept() const {
    int connection_socket_fd = ::accept(fd_, nullptr, nullptr); // blocks until a client connects, >=0 on success
    /* is this line goint to block unitl a client has connected? does it have a time out? */
    ...
    if (connection_socket_fd < 0) fail("accept");



    int yes = 1;
    ::setsockopt(connection_socket_fd,
        IPPROTO_TCP,
        TCP_NODELAY,
        &yes,
...
TcpSocket TcpSocket::connect_to(const std::string& host, uint16_t port) {
    ...
    */
    ...
    for (addrinfo* p = res; p != nullptr; p = p->ai_next) {
        ...
        if (fd < 0) continue;
        if (::connect(fd, p->ai_addr, p->ai_addrlen) == 0) break; // 0 = connected, stop
        ::close(fd);
        fd = -1;
    }
    ...
    */
    ...
    if (fd < 0) fail("connect"); //       if fd still < 0, every candidate failed
    ...
    /* so it reutrns the client' side connection socket? client side only have on socket rihgt , no listening socket */
